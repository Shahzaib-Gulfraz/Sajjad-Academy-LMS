...
type Props = {
  ...
  receivedAdminSectionTitle?: string;
  receivedTeacherSectionTitle?: string;
  adminMergedBox?: boolean;
  ...
  onDeleteAnnouncement?: (id: number | string) => void | Promise<void>;
};
...
const TeacherAnnouncements = ({
  ...
  sendSectionTitle = "Send Announcement",
  receivedSectionTitle = "Received Announcements",
  receivedAdminSectionTitle = "Admin Announcements",
  receivedTeacherSectionTitle = "Teacher Announcements",
  adminMergedBox = false,
  onAnnouncementCreated,
  hideReceived = false,
  lockTargetAll = false,
  ...
  const filteredStudents =
    announceForm.studentClassFilter === "all"
      ? students
      : students.filter((student) => student.grade === announceForm.studentClassFilter);

  const adminAnnouncements = receivedAnnouncements.filter(
    (announcement) => (announcement.authorRole ?? "").toUpperCase() === "ADMIN",
  );
  const teacherAnnouncements = receivedAnnouncements.filter(
    (announcement) => (announcement.authorRole ?? "").toUpperCase() !== "ADMIN",
  );
  ...
  const handleCreateAnnouncement = async () => {
    ...
    if (lockTargetAll || announceForm.targetType === "all") {
      targetLabel = classes.length > 0 ? `All students in ${classes.join(", ")}` : "All students";
      ...
      const studentNames = students
        .filter((student) => announceForm.selectedStudents.includes(student.id))
    ...
    const targetPayload: AnnouncementTarget = lockTargetAll
      ? { targetType: "all", targetClasses: [], targetStudentIds: [] }
      : {
  ...
  const renderAnnouncementCard = (announcement: Announcement, keyPrefix: string) => (
    <div
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-semibold text-foreground text-sm line-clamp-1">{announcement.title}</h4>
        ...
        {onDeleteAnnouncement && (
          <button
            onClick={() => void onDeleteAnnouncement(announcement.id)}
            className="inline-flex items-center rounded-md border border-border bg-destructive/10 px-3 py-1 text-xs text-destructive"
          >
            Delete
          </button>
        )}
            ...
            <div>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto border border-border rounded-xl p-3 bg-muted/5 scrollbar-thin">
                {classes.map((className) => (
                  <label key={className} className="flex items-center gap-3 text-sm cursor-pointer group">
                    <input
                      type="checkbox"
                      checked={announceForm.selectedClasses.includes(className)}
                      onChange={(e) => {
                        if (e.target.checked) {
                          setAnnounceForm({
                            ...announceForm,
                            selectedClasses: [...announceForm.selectedClasses, className],
                          });
                        } else {
                          setAnnounceForm({
                            ...announceForm,
                            selectedClasses: announceForm.selectedClasses.filter((item) => item !== className),
                          });
                        }
                      }}
                      className="rounded border-primary/30 text-primary focus:ring-primary/20 h-4 w-4"
                    />
                    <span className="text-sm text-foreground group-hover:text-primary transition-colors">{className}</span>
                  </label>
                ))}
                {classes.length === 0 && (
            ...
            <div className="space-y-3">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setAnnounceForm({ ...announceForm, studentClassFilter: e.target.value })}
                  className="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
                >
              ...
              <div>
                ...
                <div className="space-y-2 max-h-52 overflow-y-auto border border-border rounded-xl p-3 bg-muted/5 scrollbar-thin">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            ...
                            setAnnounceForm({
                              ...announceForm,
                              selectedStudents: announceForm.selectedStudents.filter((id) => id !== student.id),
                            });
                          }
                        }}
                        className="rounded border-primary/30 text-primary focus:ring-primary/20 h-4 w-4"
                  ...
                  {filteredStudents.length === 0 && (
                    <p className="text-xs text-muted-foreground text-center py-2">No students in this class.</p>
                  )}
                </div>
              </div>
            </div>
  ...
  const renderReceivedSections = () => (
    <div className="space-y-5">
      <div>
        <h4 className="text-sm font-semibold text-foreground mb-2">{receivedAdminSectionTitle}</h4>
        <div className="space-y-3">
          {adminAnnouncements.length === 0 ? (
            <p className="text-xs text-muted-foreground">No admin announcements.</p>
          ) : (
            adminAnnouncements.map((announcement) => renderAnnouncementCard(announcement, "admin"))
          )}
        </div>
      </div>

      <div>
        <h4 className="text-sm font-semibold text-foreground mb-2">{receivedTeacherSectionTitle}</h4>
        <div className="space-y-3">
          {teacherAnnouncements.length === 0 ? (
            <p className="text-xs text-muted-foreground">No teacher announcements.</p>
          ) : (
            teacherAnnouncements.map((announcement) => renderAnnouncementCard(announcement, "teacher"))
          )}
        </div>
      </div>
    </div>
      ...
      <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Create and manage announcements for your students.</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-card border border-border/60 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
          <div className="px-6 py-4 border-b border-border/60 bg-muted/10">
            <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
              <Megaphone className="h-5 w-5 text-primary" /> Announcement Box
            </h3>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-6 p-6">
            <div className="rounded-2xl border border-border/60 bg-background/40 p-5">
              <h4 className="text-base font-semibold text-foreground mb-4 flex items-center gap-2">
                <Send className="h-4 w-4 text-primary" /> {sendSectionTitle}
              </h4>
              {renderSendForm()}
            </div>
            ...
            <div className="rounded-2xl border border-border/60 bg-background/40 p-5">
              <h4 className="text-base font-semibold text-foreground mb-4 flex items-center gap-2">
                <Users className="h-4 w-4 text-primary" /> {receivedSectionTitle}
              </h4>
              <div className="max-h-[560px] overflow-y-auto scrollbar-thin pr-1">
                {renderReceivedSections()}
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl bg-card border border-border/60 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
          <div className="px-6 py-4 border-b border-border/60 bg-muted/10">
            <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
              <Send className="h-5 w-5 text-primary" /> {sendSectionTitle}
            </h3>
          </div>
          <div className="p-6">{renderSendForm()}</div>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="p-5 max-h-[400px] overflow-y-auto scrollbar-thin">
              {sentAnnouncements.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-xs text-muted-foreground/70 mt-1">Use the form to create your first announcement.</p>
                </div>
              ) : (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-muted-foreground">
                            ...
                            <span>{announcement.author}</span>
                            <span>•</span>
                            <span className="text-success/80 font-medium">{announcement.targetLabel}</span>
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          {!hideReceived && (
            <div className="rounded-2xl bg-card border border-border/60 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
              <div className="px-6 py-4 border-b border-border/60 bg-muted/10">
                <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
                  <Users className="h-5 w-5 text-primary" /> {receivedSectionTitle}
                </h3>
              </div>
              <div className="p-5 max-h-[400px] overflow-y-auto scrollbar-thin space-y-5">
